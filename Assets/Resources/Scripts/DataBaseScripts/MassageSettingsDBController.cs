...
public static class Appereance{
    ...
    public static void AddAppereanceToSqliteCommand(in SqliteCommand command){
        ...
        AddMessageColorToParams(command);
        //command.Parameters.Add(new SqliteParameter("@id", 1));   
    }

    public static void AddMessageColorToParams(in SqliteCommand command){
    ...
    public static void AddAvatarImageParams(in SqliteCommand command){
        ...
        command.Parameters.Add(new SqliteParameter("@avatar_width", Appereance.userAvatarImage.image_width));
    ...
    public static void ReadAppereanceFromReaderSqlite(IDataReader reader){
        while (reader.Read()){            
                    ReadChatBeckground(reader);
                    ReadUserAvatarImage(reader);
                    ReadChatBeckground(reader);
        }
    }

    public static void ReadUserAvatarImage(IDataReader reader){
        if(!System.DBNull.Value.Equals(reader["userAvatarByteCode"]) && 
           !System.DBNull.Value.Equals(reader["avatar_width"]) &&  
           !System.DBNull.Value.Equals(reader["avatar_height"])){
                Appereance.userAvatarImage = new ImageData((byte[])reader["userAvatarByteCode"], Convert.ToInt32(reader["avatar_width"]), Convert.ToInt32(reader["avatar_height"])); 
        }
    }

    public static void ReadChatBeckground(IDataReader reader){
        if(!System.DBNull.Value.Equals(reader["backgound_bytecode"]) && 
           !System.DBNull.Value.Equals(reader["image_width"]) && 
           !System.DBNull.Value.Equals(reader["image_height"])){
                Appereance.backgroundImage = new ImageData((byte[])reader["backgound_bytecode"], Convert.ToInt32(reader["image_width"]), Convert.ToInt32(reader["image_height"]));
        }  
    }

    public static void ReadMessageColor(IDataReader reader){
       if(!System.DBNull.Value.Equals(reader[message_color])){
...
public static class MassageSeettingsDBController{
    ...
    public static void ClearAppereanceDB(){
        using (var connection = new SqliteConnection(dbName)){
                connection.Open();
                using (var command = connection.CreateCommand()){ 
                    command.CommandText = $@"REPLACE INTO appereance_setting 
    ...
    public static void UpdateAvatar(){
        using (var connection = new SqliteConnection(dbName)){
            ...
            using (var command = connection.CreateCommand()){ 
                ...
                Appereance.AddAvatarImageParams(command);
                command.ExecuteNonQuery();
            }
            connection.Close();
        }
